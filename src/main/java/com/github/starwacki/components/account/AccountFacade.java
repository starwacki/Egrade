...
public class AccountService {
    ...
    public AccountViewDTO saveStudentAndParentAccount(AccountStudentDTO studentDTO) {
        Student student = accountFactory.createStudent(studentDTO);
        Parent parent = accountFactory.createParent(studentDTO);
    ...
    public AccountViewDTO changeAccountPassword(Role role, int id, String oldPassword, String newPassword) {
        ...
        switch (role) {
            ...
            default -> throw new IllegalOperationException(HttpMethod.PUT,role.toString());
        }
    ...
    public AccountViewDTO getAccountById(Role role, int id) {
        ...
        switch (role) {
            ...
            default -> throw new IllegalOperationException(HttpMethod.GET,role.toString());
        }
        return accountViewDTO;
    }
    ...
    public AccountViewDTO deleteAccountById(Role role,int id) {
        ...
        switch (role) {
            ...
            default -> throw new IllegalOperationException(HttpMethod.DELETE,role.toString());
        }
        return accountViewDTO;
    }

    public AccountViewDTO saveTeacherAccount(AccountTeacherDTO accountTeacherDTO) {
        Teacher teacher = teacherRepository.save(accountFactory.createTeacher(accountTeacherDTO));
    ...
    private <T extends  Account> T setPassword(JpaRepository<T,Integer> repository, T account, String newPassword) {
        account.getAccountDetails().setPassword(AES.encrypt(newPassword));
        return  repository.save(account);
    }

    private boolean arePasswordsSame(Account account, String oldPassword) {
        return AES.encrypt(oldPassword).equals(account.getAccountDetails().getPassword());
    }
